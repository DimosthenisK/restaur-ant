...
export default function SignIn({ csrfToken, message }: SignInProps) {
  const {
    register,
    handleSubmit,
    watch,
    ...
    <Header pageTitle="Sign In" breadcrumbs={["Authentication", "Sign In"]}>
      <div className="h-full pb-16 pt-4">
        <div className="flex flex-col items-center justify-center">
          ...
          {message && (
            <div className="shadow-xl rounded xl:w-1/3 lg:w-1/2 md:w-3/5 sm:w-4/5 w-full p-3 bg-red-800">
              <p className="text-center text-white text-xl">{message}</p>
            </div>
          )}

          <div
            className={`shadow-xl rounded xl:w-1/3 lg:w-1/2 md:w-3/5 sm:w-4/5 w-full p-10 bg-red-800 ${
              message ? "mt-3" : "mt-10"
            }`}
          >
            <p
